...
**A test suite shaped like the risks.** 346 unit tests over `utils/` and `api/`, plus a 78-test Playwright suite that serves every request from recorded fixtures with the page clock frozen, so a red run always means this code changed and never that a shared third-party API was slow. It asserts zero axe violations on 8 pages in both auth states, that no page scrolls sideways when every user-controlled field holds a 108-character word, and exact card counts — each carrying a comment recording what the number was when the surface was broken.
...
| Testing  | Vitest + jsdom                                   | 346 unit tests across `utils/` and `api/`.                                                     |
| E2E      | Playwright                                       | 78 fully mocked smoke tests + 16 screenshot baselines. No network, frozen clock.               |
...
**346 unit tests across 23 files** (Vitest + jsdom), concentrated where a silent bug would be
worst:
...
**78 Playwright smoke tests + 16 screenshot baselines.** Every request is served from
