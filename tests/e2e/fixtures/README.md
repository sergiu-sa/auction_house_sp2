...
| `listing-many-media.json` | `listing-single.json`, seven `media` entries | The gallery's "+N more" overflow control renders only above four images, and **no recorded fixture had more than four** — so that branch had never rendered under any test, which is how it shipped inert (F-130). Seven, not five, so the control reads `+3 more` and revealing it is distinguishable from an off-by-one. Measured live 2026-09-10: **43 of 3,200 lots (1.3%)** carry more than four, the largest recorded run being seven. |
